...
from worker_old.controller.admin import Admin as Controller
...
class Admin:
    ...
    async def _execution(self, config_job: AdminConfig, **kwargs) -> HandleExecution:
        error_flag = False
        trace_back_nfo = None
        job_event_dto = None
        ...
        try:
            job_event_dto = await Controller(self.ctx).execute(config, **kwargs)
    ...
    def _log_error(
        self, config_job: AdminConfig, results: HandleExecution
    ) -> EventError:
    ...
    def _log(self, results: HandleExecution, config_job: AdminConfig) -> Event:
    ...
    async def handle(
        self, config_job: AdminConfig, **kwargs
